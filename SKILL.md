...
- `references/27-rietveld-workflow-conventions.md` — **read this for any Rietveld/Pawley session, and always before writing a final report.** Practical defaults learned from real refinement sessions, not manual syntax: TCHZ + `CuKa2_analyt` as the standing peak-shape/wavelength pairing for unmonochromated lab Cu data (applies to both `str` and `hkl_Is`); always seeding a placeholder `r_wp`/`r_exp`/`r_p`/`gof` line at the top of a brand-new `.inp` so refinement statistics stay visible at a glance; the exact recipe for generating a Pawley `hkl_Is` reflection list from an already-converged `str` model via `Create_hklm_d_Th2_Ip_file`, for a fair Rietveld-vs-Pawley Rwp comparison; and mandatory final-report formatting (date first, refined-parameter count split structural/non-structural, a fractional-coordinates table with errors in parens, a fixed closing disclaimer line) that applies regardless of instrument. Has placeholder headings for synchrotron and neutron (CW/TOF) conventions, not yet populated.
