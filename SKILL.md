...
- `references/27-rietveld-workflow-conventions.md` — **read this for any Rietveld/Pawley session, and always before writing a final report.** Practical strategy conventions learned from real refinement sessions, not manual syntax — wavelength/monochromator handling, which of the four peak-shape families to use and how they pair with wavelength macros and `Simple_Axial_Model()`, 2-theta range strategy, ADP strategy, a full staged-refinement sequencing order, how to handle a peak-shape term that saturates, a mandatory false-minimum re-check before finalizing any correlated peak shape, the Pawley-from-converged-`str` recipe, mandatory final-report formatting, and a dedicated quantitative-phase-analysis section (ADP handling exceptions, phase screening/elimination, `weight_percent`/`elemental_composition`, the final pie chart). **Every concrete rule carries a stable citable tag** (`(R1)`, `(R2)`, … ) in one continuously-increasing sequence for that file, so a rule can be referenced, changed, or superseded precisely in a future session without re-deriving or re-describing it — see that file's own intro for the numbering convention. Don't cite specific rule numbers from here in `SKILL.md`, since they renumber independently of this file and any number written here would go stale; point to the rule by its topic/description instead and look up the current number directly in `27-...` itself. Has placeholder headings for synchrotron and neutron (CW/TOF) conventions, not yet populated.
...
- `scripts/plot_sh_sphere.py` — **run this whenever the user asks to see/plot/draw a spherical-harmonics series as a pole figure or 3D surface** — a preferred-orientation/texture correction (`PO_Spherical_Harmonics`) or any other `spherical_harmonics_hkl` use (e.g. the anisotropic-broadening form under hkl-dependent peak shapes in `references/27-rietveld-workflow-conventions.md`). Renders one `str` block's series as a self-contained interactive HTML pole figure (drag to orbit, scroll to zoom), no third-party dependencies. **Carries an on-page "not extensively tested" warning banner — keep it there until this has more mileage.**
  ```
  python3 scripts/plot_sh_sphere.py file.inp                     # -> file_sh_sphere.html, raw dots
  python3 scripts/plot_sh_sphere.py file.inp --surface           # + interpolated surface / contour / radius toggles
  python3 scripts/plot_sh_sphere.py file.inp --phase 2 -o tex.html
  python3 scripts/plot_sh_sphere.py file.inp --surface --mesh-level 4   # finer mesh, slower
  ```
...
- `scripts/plot_xy.py` — **run this whenever the user asks to plot a raw XY(E) data file** (`.xy`, `.xye`, `.dat`, or `Out_X_Yobs`/`Out_X_Ycalc` output). Renders a self-contained interactive HTML page: drag to pan, scroll to zoom (both cursor-centered), a per-series visibility checkbox in the legend, a hover crosshair (no floating tooltip box), a Sqrt(Y)/Linear toggle for compressing dynamic range, and an optional `--stats "Rwp=13.68%,GoF=1.64"` (comma-separated `Label=value` pairs) to show refinement statistics as plain text in the header next to the point-count/X-range meta line.
  ...
  **"Run and plot [file]" is a standing workflow**: (1) run the file via `tc.exe` on a scratch copy, never the original in place; (2) if `Out_X_Ycalc(...)`/`Out_X_Yobs(...)` aren't already present, warn and add them to the scratch copy; for a Rietveld/Pawley fit, also add `Create_hklm_d_Th2_Ip_file(ticks.txt)` inside the `str`/`hkl_Is` block if not already present, so reflection tick marks with real h k l hover labels are available by default, not just on request (see `references/27-rietveld-workflow-conventions.md`'s rule on default reflection ticks — one tick file per phase for a multi-phase fit); (3) plot Yobs, Ycalc, and their difference together, passing the run's own `r_wp`/`gof` (and `r_p` if relevant) from the resulting `.out` via `--stats "Rwp=13.68%,GoF=1.64"` so the fit quality is visible directly in the plot header rather than requiring a separate look at the `.out` file (see that same reference file's rule on plot stats), and the tick file(s) via `--ticks "NAME|ticks.txt,..."`; (4) if the file has more than one `xdd` block, produce one plot per `xdd` by default — unless the user says **"fit show in one plot"**, in which case combine every phase via `--phases` (tick groups still combine via `--ticks`, one per phase). Two general facts worth remembering for any multi-phase file: TOPAS enforces one shared value per parameter name globally (reusing e.g. `CS_L(csl, ...)` at different values across two phases aborts the run with `Parameter csl is defined more than once...`); and a plain relative data filename in a `DAT`/`XDD`/`RAW` macro call resolves relative to the referencing `.inp` file's own directory (same rule as `#include`), so a scratch copy run from elsewhere needs its data file alongside it.

- `scripts/build_report_docx.py` — **use this if the user wants the final refinement report as a `.docx` instead of (or alongside) plain markdown.** Fills the styles-only `templates/refinement_report_template.docx` (Arial 10pt body, styled headings, bordered/shaded table style, disclaimer style) so every report looks the same regardless of refinement type — don't hand-format a `.docx` or reach for `pandoc` (not guaranteed installed). Call `build_report(title, date, sections, out_path, images=..., plots=..., files=...)` — see the script's own module docstring for the full argument shapes, content conventions (e.g. use "Å" not "A"), and a worked example. To change the look (font/size/colors), edit the named styles in the template `.docx` directly in Word; the script only refers to styles by name, so no code change is needed. Don't add visible content to the template itself — it's loaded as every report's base document, so anything in its body leaks into every generated report (`scripts/_build_template.py` is the one-off generator if it must be rebuilt). To put an actual Rietveld-fit plot image in the document (not just a hyperlink), render it first with `scripts/plot_xy_matplotlib.py` (same flags as `plot_xy.py`, but PNG/JPG output instead of an interactive HTML canvas) and pass the resulting path via `images=[("caption", "fit.png")]`.
...
- `scripts/to_Reel_v1.py` — converts TOPAS `.xyd` Rietveld-fit output into `.xyy` files for viewing in Reel. The `.xyd` format itself is written by the `Write_Rietveld_xyd`/`Writephase` macro pair — see `example_inp_files/d8_01612_vt_reel_02.inp` for a full worked sequential/VT example that writes them. **Unlike the other scripts here, this one is not run directly from this folder** — copy it into the project's own working directory (next to the `.inp`) before running, since the normal way to invoke it is from inside the `.inp` itself via `system_after_save_OUT { python to_Reel_v1.py <results_dir> }`, which shells out relative to wherever `tc.exe` is running, not this skill's own path.
  ...
  One side effect worth knowing before running: if `write_background_subtracted` is turned on, it creates a `data_background_subtracted/` directory **in the current working directory** (a hardcoded name, not a CLI option) — off by default. `delete_xyd` defaults to `True`, so the source `.xyd` files are removed once their `.xyy` has been written. Multiprocessed (`cpu_count()-1` workers), so it's fine to point at a large VT/sequential batch's worth of `.xyd` files at once.
