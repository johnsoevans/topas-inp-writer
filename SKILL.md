...
- `scripts/check_inp_syntax.py` — a heuristic syntax checker. Run it against any `.inp` file or directory before handing it over:
  ...
  **Pass every file you want checked to a single invocation** (multiple paths, or a directory) rather than calling the script once per file. Each run spends ~200 ms loading the reference keyword corpus and the `.inc` macro library before it looks at any file, and that load is shared across all files in one invocation.
...
- `scripts/find_example.py` — **run this whenever the user asks for a template/example/starting point for some refinement topic**, e.g. "tof template". Searches every `.inp` under `TOPAS_DIR` (~312 on a v8 install) plus this skill's own `example_inp_files/`. Matches are ranked in three tiers: `example_inp_files/`, then install examples listed in `references/examples-index.md`, then everything else. Tier breaks ties on match score rather than overriding it, so a file matching two query words still outranks a better-tiered file matching one. Scratch files (0-byte, `temp*.inp`, `... - Copy.inp`) are dropped. With `TOPAS_DIR` unset it searches `example_inp_files/` alone. `--bat <tcinps-2.bat>` searches a curated tc-list instead of walking `TOPAS_DIR`; no such file is bundled with this skill.
  ```
  ...
  ```
  Two matching passes: (1) whole-token path match — the path is split on every non-alphanumeric character, so "tof" matches both `tof/tof_bank2_1.inp` and `hrpd_tof_rietveld.inp` but not `jsoe_fit_cc2c_tofullprofmono_01.inp`; a query naming a template scores +1 for any path with a `template` token; (2) a content match against a curated topic-synonym list (Pawley, Le Bail, indexing, PDF, charge-flipping, quant, stacking faults, rigid bodies, deconvolution, magnetic, protein, single-crystal, TOF, parametric/sequential) for queries naming a concept the corpus only abbreviates (e.g. "charge flipping" → `cf/`). Pass (2) reads every file in the corpus, so it runs only when pass (1) returns fewer than 5 hits. Deliberately does NOT auto-genericize a found example into a placeholder-filled template — deciding what's an instrument constant to keep vs. a sample-specific value to placeholder-ize needs real per-topic judgment; `example_inp_files/tof_template.inp` is what that judgment call looks like once applied by hand.
...
**3. Check unexplained weak peaks against Cu Kβ and W Lα contamination.** Both originate in the source, not the sample, and both fall at *lower* 2θ than the Kα1 peak they derive from: `sin(θ) = sin(θ_Kα1) × λ/λ_Kα1`, with λ_Kβ = 1.39222 Å (ratio 0.9037) and λ_W Lα = 1.47642 Å (ratio 0.9583, from an aged tube). Each is normally only a few percent of its parent, so they appear only for the strongest Cu Ka reflections. Unlike a Kα2 shoulder they sit well clear of the parent peak. Exclude them from the peak list — indexing a contamination peak forces a wrong cell.

**4. After fitting, generate the calculated pattern with `Out_X_Ycalc(file)` and overlay it against the observed data — don't trust Rwp alone.** Add inside the `xdd { }` block after the `xo_Is`/`load xo I {}` block:
```
...
**5. Restrict `start_X`/`finish_X` to the region with usable peaks, and stop `finish_X` before peaks become poorly resolved.**
```
xdd pbso4.raw
   start_X 15.94
   finish_X 65
