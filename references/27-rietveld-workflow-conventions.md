...
**(R15) If the starting model is good, refining everything from the start is often safe.** For a more complex refinement, or whenever in doubt, use this staged order instead. (This allowance applies to R16-R22 only — the order in which structural/peak-shape parameters are freed. It does not waive R11's restricted-range requirement, which guards against a different failure mode and applies regardless of starting-model quality.)
